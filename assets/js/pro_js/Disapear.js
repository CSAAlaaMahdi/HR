Disapear_fetch();
Disapear_filldata();
Disapear_Permissions();

function Disapear_cleardata() {
    ...
    $("#ndirection").dxSelectBox("instance").option("value", "");
    $("#docno").dxTextBox("instance").option("value", "");
    $("#docdate").dxDateBox("instance").option("value", "");
    $("#duration_from").dxDateBox("instance").option("value", "");
    $("#duration_to").dxDateBox("instance").option("value", "");
    $("#notes").dxTextArea("instance").option("value", "");
    $("#FilePath").dxFileUploader("instance").option("value","");
    $("#Disapearimage-container").empty();

}

function Disapear_chechdata() {
    if ($.trim($("#rtype").dxSelectBox("instance").option('value')).length == 0) {
        error_rtype = " مطلوب ";
    ...
    if ($.trim($("#ndirection").dxSelectBox("instance").option('value')).length == 0) {
        error_ndirection = " مطلوب ";
        $("#error_ndirection").text(error_ndirection);
    } else {
        error_ndirection = "";
        $("#error_ndirection").text(error_ndirection);
    }
...
function Disapear_UpdateOrCreate() {
    var url = "disapear";
    ...
    var selecteddurationfrom = $("#duration_from").dxDateBox("instance").option("value");
    var duration_from = new Intl.DateTimeFormat("en-US", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
    }).format(selecteddurationfrom);
    var selecteddurationto = $("#duration_to").dxDateBox("instance").option("value");
    var duration_to = new Intl.DateTimeFormat("en-US", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
    ...
    formData.append('ndirection', $("#ndirection").dxSelectBox("instance").option("value"));
    ...
    formData.append('DocTitle', $("#rtype").dxSelectBox("instance").option("value"));
...
function Disapear_fetch() {
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                $(function () {
                    const dataGrid = $("#Disapeardatagrid").dxDataGrid({
                        dataSource: response.getDisapear,
                        keyExpr: "rid",
                        paging: {
                            enabled: true,
                        ...
                        columns: [
                            ...
                            },
                            {
                                ...
                                cellTemplate: function (container, options) {
                                    var imageUrl = 'assets/img/navbar/icons8_leaving_queue_64px.png' ;
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (response) {
                                                    ...
                                                    $("#rid")
                                                        .dxTextBox("instance")
                                                        .option({
                                                            value: response.Disapear.rid,
                                                        });
                                                        $("#Guid")
                                                        .dxTextBox("instance")
                                                        .option({
                                                            value: response.Disapear.Guid,
                                                        });
                                                    $("#eid")
                                                        .dxDropDownBox("instance")
                                                        .option({
                                                            value: Number(response.Disapear.eid),
                                                        });
                                                    $("#rtype")
                                                        .dxSelectBox("instance")
                                                        .option({
                                                            value: response.Disapear.rtype,
                                                        });
                                                    $("#ndirection")
                                                        .dxSelectBox("instance")
                                                        .option({
                                                            value: response.Disapear.ndirection,
                                                        });

                                                    $("#docno")
                                                        .dxTextBox("instance")
                                                        .option({
                                                            value:response.Disapear.docno
                                                        });
                                                        $("#docdate")
                                                        .dxDateBox("instance")
                                                        .option({
                                                            value:new Date(response.Disapear.docdate)
                                                        });
                                                        $("#duration_from")
                                                        .dxDateBox("instance")
                                                        .option({
                                                            value:new Date(response.Disapear.duration_from)
                                                        });
                                                        $("#duration_to")
                                                        .dxDateBox("instance")
                                                        .option({
                                                        ...
                                                        $.each(response.Attachments, function(index, file) {
                                                          ...
                                                          // Delete Image
                                                        $('#Disapearimage-container').on('click', '.delete-image', function() {
                                                            ...
                                                            if(index >=0 && index < images.length){
                                                                ...
                                                                $.ajax({
                                                                    url: 'disapearDelete/DeleteImage', // Replace 'deleteImage' with your actual backend endpoint
                                                                    method: 'POST',
                        ...
                        onContentReady: function (e) {
                            ...
                            e.element
                                ...
                                .addClass("custom-header_Disapear");
                        },
                    });
                });
            },
...
function Disapear_filldata() {
    var url = "disapearfill/";
    $(document).ready(function () {
        $.ajax({
            type: "GET",
            url: url + "filldata",
            success: function (response) {
                $(() => {
                    $('#rtype').dxSelectBox({
                        dataSource: response.getDisapearType,
                        ...
                        onCustomItemCreating(data) {
                                ...
                                response.getDisapearType.push(newItem);
                ...
                $(() => {
                    $('#ndirection').dxSelectBox({
                        dataSource: response.getDisapearDirection,
                        inputAttr: {style:"font-size:13px", },
                        placeholder:"ادخل  الجهة المستفيدة",
                        searchEnabled:true,
                        displayExpr: 'ndirection',
                        valueExpr: 'ndirection',
                        searchMode: "contains",
                        acceptCustomValue: true,
                        dropDownOptions: {
                            height: 400
                        ...
                        onCustomItemCreating(data) {
                                ...
                                const newItem = {
                                    ndirection: data.text
                                };

                                response.getDisapearDirection.push(newItem);
                                data.component.option("value",newItem);
                                data.customItem = newItem;
...
$(document).ready(function () {
    $("#danger-contained").dxButton({
        ...
        width: 120,
        onClick() {
            var displaycard = document.getElementById("Disapearaction");
            if (displaycard.style.display == "block") {
                document.getElementById("card_Disapeartitle").innerText = "";
                Disapear_cleardata();
                // Disapear_setStCode();
    ...
    $("#btnNewAdd").dxButton({
        ...
        onClick() {
            ...
            if (displaycard.style.display == "block") {
                ...
                document.getElementById("card_Disapeartitle").innerText ="اضافة  حركة ملاك";
                displaycard.style.display = "block";
                document.getElementById("card_Disapeartitle").scrollIntoView();
...
$(document).ready(function () {
    $("#btnSave").dxButton({
        ...
        onClick() {
            Disapear_chechdata();
            if(
                error_rtype != "" || error_ndirection != ""
            )
            {
                return false;
            }else{
                Disapear_UpdateOrCreate();
            }
        },
    });
});
